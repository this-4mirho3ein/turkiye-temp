      ...
      {isAgencyOwner && (
        <div className="flex justify-end mb-4">
          <button
            ...
            className="flex items-center gap-1 px-4 py-2 text-white transition-colors bg-purple-600 rounded-lg shadow-sm hover:bg-purple-700"
          >
            <svg
      ...
      {consultants.length === 0 ? (
        <div className="py-8 text-center text-gray-500">
          ...
          {isAgencyOwner && (
            <button
              onClick={onAddConsultant}
              className="inline-flex items-center gap-1 px-4 py-2 mt-4 text-white transition-colors bg-purple-600 rounded-lg hover:bg-purple-700"
            >
              <svg
                xmlns="http://www.w3.org/2000/svg"
                width="18"
            ...
            <thead className="bg-gray-50">
              <tr>
                ...
                <th
                  scope="col"
                  className="px-6 py-3 text-xs font-medium tracking-wider text-right text-gray-500 uppercase"
                  style={{ width: isAgencyOwner ? "25%" : "35%" }}
                >
                  ایمیل
                </th>
                {isAgencyOwner && (
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {consultants.map((consultant) => (
                <tr key={consultant._id} className="hover:bg-gray-50">
                  ...
                  <td
                    className="px-6 py-4 text-sm text-right text-gray-500"
                    style={{ width: "30%" }}
                    dir="ltr"
                  >
                    <div className="truncate">{consultant.phone || "-"}</div>
                  </td>
                  <td
                    className="px-6 py-4 text-sm text-right text-gray-500"
                    style={{ width: isAgencyOwner ? "25%" : "35%" }}
                    dir="ltr"
                  >
                    <div className="truncate">{consultant.email || "-"}</div>
                  </td>
                  {isAgencyOwner && (
                    <td
                      className="px-6 py-4 text-sm text-right text-gray-500"
