...
import { addAgencyAreaAdmin } from "@/controllers/makeRequest";
import { addToast } from "@heroui/react";
import { successMessage } from "@/utils/showMessages";
import { z } from "zod";

interface AddAreaAdminModalProps {
  isOpen: boolean;
  onClose: () => void;
  agencyId: string;
...
const AddAreaAdminModal: React.FC<AddAreaAdminModalProps> = ({
  isOpen,
  onClose,
  agencyId,
  isAgencyOwner,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // API call to add area admin
      const response = await addAgencyAreaAdmin(agencyId, validatedPhone);
      
      if (response.success) {
        // Show success toast notification with message from API
        addToast({
          title: 'موفق', // Success
          description: response.message || 'مدیر منطقه با موفقیت اضافه شد',
          color: 'success',
          radius: 'md',
          variant: 'solid',
          timeout: 3000,
        ...
        addToast({
          title: 'خطا', // Error
          description: response.message || 'خطا در افزودن مدیر منطقه',
          color: 'danger',
          radius: 'md',
          variant: 'solid',
          timeout: 3000,
        ...
        setError(response.message || "خطا در افزودن مدیر منطقه");
      }
    } catch (err: any) {
      if (err instanceof z.ZodError) {
        ...
        addToast({
          title: 'خطا', // Error
          description: err.message || 'خطا در افزودن مدیر منطقه',
          color: 'danger',
          radius: 'md',
          variant: 'solid',
          timeout: 3000,
        ...
        setError(err.message || "خطا در افزودن مدیر منطقه");
      }
    } finally {
      setLoading(false);
    }
            ...
            >
              ...
              <div className="bg-gradient-to-r from-purple-50 to-indigo-50 px-6 py-4 border-b border-gray-100">
                <div className="flex justify-between items-center">
                  <h3 className="text-xl font-bold text-gray-800">افزودن مدیر منطقه</h3>
                  <button
                    onClick={onClose}
                ...
                <form onSubmit={handleSubmit}>
                  <div className="mb-6">
                    ...
                    <p className="mt-2 text-gray-500 text-sm">
                      شماره تلفن کاربری که می‌خواهید به عنوان مدیر منطقه اضافه کنید را وارد کنید.
                    </p>
                  </div>
...
export default AddAreaAdminModal;
